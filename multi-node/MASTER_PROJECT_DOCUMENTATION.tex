...
\fbox{
    \begin{minipage}{0.8\textwidth}
        \centering
        \vspace{8cm}
        \textbf{--- [PLACEHOLDER] INSERT SYSTEM ARCHITECTURE TIKZ CODE HERE ---}
        \vspace{8cm}
    \end{minipage}
}
\caption{System Architecture Diagram (User Self-Design)}
\end{figure}
...
\fbox{
    \begin{minipage}{0.8\textwidth}
        \centering
        \vspace{6cm}
        \textbf{--- [PLACEHOLDER] INSERT SEQUENCE DIAGRAM TIKZ CODE HERE ---}
        \vspace{6cm}
    \end{minipage}
}
\caption{Security Event Lifecycle Sequence (User Self-Design)}
\end{figure}
...
\fbox{
    \begin{minipage}{0.8\textwidth}
        \centering
        \vspace{6cm}
        \textbf{--- [PLACEHOLDER] INSERT WORKFLOW DIAGRAM TIKZ CODE HERE ---}
        \vspace{6cm}
    \end{minipage}
}
\caption{Enterprise Resiliency Workflow (User Self-Design)}
\end{figure}


\subsubsection{3.4 Tier 1: The Resilience Layer (Networking \& HA)}
\begin{itemize}
\item   \textbf{Virtual IP (VIP)}: \texttt{172.25.0.222} - The single entry point for all traffic.
...
\item   For the Dashboard, Grafana, and Zabbix UI, Nginx acts as a standard Reverse Proxy, terminating SSL (if configured) and adding \texttt{X-Forwarded-For} headers so the backends see the real client IP, not the Docker bridge IP.
\end{itemize}
