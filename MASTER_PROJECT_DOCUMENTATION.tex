...
\lstset{
    ...
    keywordstyle=\color{soc-blue}
}

% (Kept for reference; not used on cover page)
\title{\textbf{\Huge Enterprise-Grade Wazuh SIEM} \\ \vspace{0.5cm} \Large The Ultimate Whitepaper Edition}

\begin{document}

% ======================================================
% 1) COVER PAGE (FULL PAGE IMAGE) - PAGE 1
% File name: Enterprise_Grade_Wazuh_SIEM.png
% ======================================================
\newgeometry{a4paper, margin=0in} % no margins for cover
\thispagestyle{empty}
\noindent\includegraphics[width=\paperwidth,height=\paperheight]{Enterprise_Grade_Wazuh_SIEM.png}
\clearpage
\restoregeometry % back to your normal margins (0.6in)
...
\textbf{Version:} 3.1.5 (Enterprise Stability Edition) \\
\textbf{Classification:} Enterprise Technical Manual \\
\textbf{Repository:} 
\href{https://github.com/SaeidSec/Docker-base-Enterprise-Grade-Wazuh-SIEM-Advanced-Load-Balancing-and-High-Availability-Architecture}{GitHub} \\
...
\begin{center}
\textit{
This whitepaper is dedicated to my wife, \\
whose unwavering support, patience, and belief \\
...
\begin{figure}[ht!]
    \centering
    \includegraphics[width=0.9\textwidth]{Wazuh_SIEM_High_Availability_Architecture.png}
    \caption{Wazuh SIEM High Availability Architecture}
    \label{fig:wazuh-ha-architecture}
\end{figure}
...
\begin{figure}[ht!]
    \centering
    \includegraphics[width=0.9\textwidth]{Wazuh_Security_Event_Lifecycle.png}
    \caption{Wazuh Security Event Lifecycle Sequence}
    \label{fig:wazuh-event-lifecycle}
\end{figure}
...
\begin{figure}[ht!]
    \centering
    \includegraphics[width=0.9\textwidth]{Wazuh_HA_Failover_Workflow.png}
    \caption{Enterprise Resiliency and High Availability Failover Workflow in Wazuh}
    \label{fig:wazuh-ha-failover}
\end{figure}
...
\item   \textbf{Infrastructure Nodes}: 2x Alpine "Anchor" nodes holding the network namespaces.
\end{itemize}
...
\begin{itemize}
    ...
    \item   For Wazuh Agent traffic (1514), we use \textbf{Consistent Hashing} (\texttt{hash \$remote\_{}addr consistent}). This ensures that a specific agent always reconnects to the same Manager worker unless that worker is down.
    \item   \textbf{L7 Proxying \& Hardening (HTTP)}:
    \item   For management UIs (Dashboard, Grafana, Zabbix), Nginx provides advanced traffic control:
    \begin{longtable}{@{}llp{8cm}@{}}
        \toprule
        \textbf{Hardening Category} & \textbf{Setting} & \textbf{Technical Purpose} \\
        \midrule
        \textbf{Session Integrity} & \texttt{WebSocket Upgrades} & Supports Grafana Live and real-time socket streams. \\
        \textbf{Protocol Awareness} & \texttt{X-Forwarded-Proto} & Ensures backends correctly handle HTTPS redirection logic. \\
        \textbf{Fault Tolerance} & \texttt{max\_{}fails=3} & Automatically masks transient backend failures from the user. \\
        \textbf{Timeout Resilience} & \texttt{proxy\_{}read\_{}timeout 60s} & Prevents hanging UI sockets during heavy indexer query loads. \\
        \bottomrule
    \end{longtable}
\end{itemize}

\subsubsection{5.4 Configuration Abstraction: The \texttt{wazuh.vip} Pattern}
To eliminate the fragility of hardcoded IPs, we implemented an abstraction layer within the deployment logic.
\begin{itemize}
    \item   \textbf{The problem}: Hardcoded IPs in container environment variables require global updates if the network fabric changes.
    \item   \textbf{The Solution}: We utilized \texttt{extra\_{}hosts} to map the Keepalived VIP (\texttt{172.25.0.222}) to a logical internal hostname: \texttt{wazuh.vip}.
    \item   \textbf{Impact}: Core services now connect to \texttt{https://wazuh.vip}, ensuring that infrastructure changes only require a single update at the host mapping level, following the Principle of Least Knowledge (PoLK).
\end{itemize}

\subsubsection{5.5 Keepalived VRRP Logic}
\begin{itemize}
...
\subsection{Phase 1: Security Audit \& Vulnerability Identification}
...
\subsection{Phase 2: Implementation \& Remediation (Hardening Solutions)}
...
\subsection{6.3 Phase 3: Resilience Optimization \& Agent Key Persistence}
...
\subsubsection{Solution 1: Forced Agent Re-registration}
To ensure that agents can always recover their connection even if local keys become desynchronized during a failover, the Wazuh Manager's \texttt{auth} configuration was optimized:
\begin{itemize}
    \item \textbf{\texttt{force\_insert}}: Enabled to allow the manager to overwrite existing agent records with the same name if a new registration request is received.
    \item \textbf{\texttt{force\_time}}: Set to \texttt{0} to allow immediate re-registration without a timeout penalty.
\end{itemize}
This ensures that the "Duplicate Name" error never blocks a legitimate agent from re-joining the cluster.

\subsubsection{Solution 2: Persistent TCP Session Tuning}
This provides a "warm" buffer that allows the cluster components to recover before the network layer drops the session.

\subsection{6.4 Phase 4: Graylog Enterprise SSL \& JVM Tuning}
The final hardening phase optimized the Graylog 7.0 stack for enterprise-grade reliability and SSL integrity. \\

\begin{enumerate}
    \item \textbf{Full SSL Verification}: 
    Transitioned Graylog's connection to the Indexer Cluster from \textit{none} to \textit{full} verification. This ensures Graylog rejects any untrusted communication, fulfilling strict compliance requirements.
    
    \item \textbf{JVM Heap Optimization}: 
    Increased Graylog's JVM heap from 1GB to \textbf{2GB} (\texttt{-Xms2g -Xmx2g}). This prevents Garbage Collection (GC) pauses during high-volume log ingestion.
    
    \item \textbf{Cross-Origin (CORS) \& VIP Persistence}: 
    Enabled \texttt{GRAYLOG\_HTTP\_ENABLE\_CORS=true} and standardized \texttt{GRAYLOG\_HTTP\_PUBLISH\_URI} to the VIP, eliminating "under-provisioned" warnings.
\end{enumerate}

--- \\

\section{7. Full-Stack Observability Ecosystem}
...
\subsection{11.2 Universal Deployment Sequence ("One-Click")}
To ensure a perfect installation on any Linux distribution, we have engineered a "Super Fixer" bootstrap script that automates the complex setup lifecycle.

\begin{enumerate}
\item \textbf{Preparation}: Clone the repository and navigate to the \texttt{multi-node} directory.
\item \textbf{Execution}: Run the Universal Fixer with root privileges:
\begin{lstlisting}[language=bash]
sudo bash bootstrap.sh
\end{lstlisting}
\item \textbf{Automated Tasks}: The script will automatically perform:
\begin{itemize}
    \item System kernel tuning (\texttt{vm.max_map_count}).
    \item SSL Certificate generation via the generator service.
    \item Environment file (\texttt{.env}) creation and validation.
    \item Dynamic Docker volume cleanup to prevent mount errors.
    \item High-Availability stack deployment.
    \item \textbf{Template Auto-Loading}: The script waits for the Indexer cluster to be healthy and then force-uploads the Wazuh index template from the Manager.
\end{itemize}
\item \textbf{Verification}: Once the script displays \texttt{ALL FIXED!}, the dashboard is ready for immediate use.
\end{enumerate}

\begin{itemize}
    \item \textbf{Permission Fixes}: The script automatically applies \texttt{chmod 644} to SSL certificates, ensuring non-root services (like Graylog UID 1100) can read the truststore and PEM files.
\end{itemize}
